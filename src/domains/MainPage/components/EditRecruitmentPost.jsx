        ...
        const fetchRecruitmentData = async () => {
            try {
                ...
                if (postData.projectId) {
                    const stackResponse = await api.get(`/projects/${postData.projectId}/stacks`);
                    setStacks(stackResponse.data.map(s => s.stack?.name || "스택 없음"));
                }
                ...
                // 첨부파일 불러오기
                const attachmentResponse = await api.get(`/recruitments/${recruitmentId}/attachments`);
                console.log("attachmentResponse.data", attachmentResponse);
                
                const fetchedAttachments = attachmentResponse.data.map(attachment => ({
                    type: 'file',
