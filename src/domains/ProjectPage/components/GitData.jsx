...
const GitData = () => {
  ...
  const sortedCommits = GitData.flatMap((repo) =>
  ...
  const sortedPulls = GitData.flatMap((repo) =>
    repo.pulls.map((pull) => ({
      type: "pull",
      id: pull.id,
      title: pull.title,
  ...
  const sortedIssues = GitData.flatMap((repo) =>
  ...
  const filterData = (data) => {
    ...
    return data.filter((item) => {
      const isGithubUrl1 =
        filter === "data.githubUrl1" && item.firstUrl === true;
      const isGithubUrl2 =
        filter === "data.githubUrl2" && item.firstUrl === false;
      return (
        (filter === "data.githubUrl1" && isGithubUrl1) ||
          ...
          <Box sx={{ marginBottom: 2 }}>
            <FormControl sx={{ minWidth: 120 }}>
              ...
              >
                ...
                {gitUrls[0] && (
                  <MenuItem value="data.githubUrl1">{gitUrls[0]}</MenuItem>
                )}
                {gitUrls[1] && (
                  <MenuItem value="data.githubUrl2">{gitUrls[1]}</MenuItem>
                )}
              </Select>
            </FormControl>
          </Box>
              ...
              >
                ...
                >
                  {filteredCommits.map((commit, i) => (
                    <Box
                      onClick={() =>
                        (window.location.href = commit.firstUrl
                          ? gitUrls[0]
                          : gitUrls[1])
                      }
                      key={`commit-${i}`}
              ...
              >
                ...
                >
                  {filteredPulls.map((pull, i) => (
                    <Box
                      onClick={() =>
                        (window.location.href = pull.firstUrl
                          ? gitUrls[0]
                          : gitUrls[1])
                      }
                      key={`pull-${i}`}
                ...
                >
                  {filteredIssues.map((issue, i) => (
                    <Box
                      onClick={() =>
                        (window.location.href = issue.firstUrl
                          ? gitUrls[0]
                          : gitUrls[1])
                      }
                      key={`issue-${i}`}
