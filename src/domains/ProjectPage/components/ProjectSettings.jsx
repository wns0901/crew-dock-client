...
const ProjectSettings = () => {
  ...
  const isCaptain = projectRoles.some(
    (role) =>
      Number(role.projectId) === Number(projectId) && role.role.isCaptain
  );
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((response) => {
        ...
        const formattedStacks =
          projectData.stacks?.map((stackItem) => stackItem.stack?.id) || [];
    ...
    api
      ...
      .then((response) => {
        ...
        const captainMember = members.find(
          (member) => member.authority === "CAPTAIN"
        );
        if (captainMember) {
        ...
        <Autocomplete
          ...
          value={
            updatedProject.stacks
              ...
              .filter((stack) => stack !== undefined) // undefined 값 필터링
          }
          onChange={handleStackChange}
          getOptionLabel={(option) => option?.name || ""} // option이 없을 경우 빈 문자열 반환
          renderTags={(value, getTagProps) =>
            value.map((option, index) => (
              <Chip
                label={option?.name || ""} // option이 없을 경우 빈 문자열 반환
                {...getTagProps({ index })}
                key={option?.id || index} // option이 없을 경우 index를 key로 사용
              />
            ))
          }
          renderInput={(params) => (
